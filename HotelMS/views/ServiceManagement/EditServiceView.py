# views/ServiceManagement/EditServiceView.py (MODIFIED)
import sys
import os
...
class EditServiceView(QDialog):
    """Dialog for editing an existing service with controller integration."""
    
    service_updated = pyqtSignal(dict)  # Signal when service is updated
    ...
    def _build_ui(self):
        ...
        # ── HEADER ─────────────────────────────────────────────────────────
        header = QWidget()
        header.setObjectName("edit_header")
        header.setStyleSheet(
            "QWidget#edit_header { background-color: #FFE0E3; border: none; }")
        header.setFixedHeight(48)

        header_layout = QHBoxLayout(header)
        header_layout.setContentsMargins(20, 10, 20, 10)

        title = QLabel("Edit Service")
        ...
        # ── FORM BODY ──────────────────────────────────────────────────────
        body = QWidget()
        body.setObjectName("edit_body")
        body.setStyleSheet("QWidget#edit_body { background-color: #2F2038; border: none; }")
    ...
    def handle_update(self):
        ...
        if self._service_id is None:
            QMessageBox.critical(self, "Error", "No service selected for editing.")
            return
        ...
        # Call controller
        success, message = self.controller.update(self._service_id, name, price, duration, status)
        
        if success:
            # Emit signal with updated service data
    ...
    # ── PUBLIC METHODS ───────────────────────────────────────────────────────
    def load_service_data(self, service_id, name: str, price: str,
                          duration: str, status: str):
        """Load service data into form fields."""
        self._service_id = service_id
        self.service_name_input.setText(name)
        self.service_price_input.setText(str(price))
        self.service_duration_input.setText(str(duration))
...
if __name__ == "__main__":
    ...
    dlg = EditServiceView()
    dlg.load_service_data(1, "Massage", "500", "60", "Active")
    dlg.show()
    sys.exit(app.exec())